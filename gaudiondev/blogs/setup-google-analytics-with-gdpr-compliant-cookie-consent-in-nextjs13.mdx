...
First step is to setup Google Analytics. To follow along with this post, you'll need the **Measaurement ID** of a Web Data Stream.
...
We're first going to build a Google Analytics component which will store the basic gtag.js code which iteracts with Google Analytics.
...
Within the second script we can see the newly added "consent" section that will default the `analytics_storage` cookeis to denied. This is using [Google Analytics Consent Mode](https://support.google.com/analytics/answer/9976101) and we can then override this consent once the user accepts cookies.
...
// app/layout.tsx
<html lang="en">
    <GoogleAnalytics GA_MEASUREMENT_ID='G-9N7KFV3730'/>
    <body>{children}</body>
</html>
```
...
To start will, we'll add a new component called Cookie Banner which will allow our users to opt-in or opt-out of cookies.
...
With our component built, we can now add functionality to it to opt-in or out of cookies.
...
Within the second block, we also save the preferences back to localstorage so that we don't have to ask the user everytime.
...
And that's it - you now have a cookie banner setup that prompts for Cookie Consent, using Google Analytics. If you found this tutorial helpful, feel free to share it and if you have any questions, feel free to [Get In Touch](https://twitter.com/Ryan_Gaudion)